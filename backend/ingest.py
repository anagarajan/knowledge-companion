...
def main():
    ...
    # ── Remove mode ───────────────────────────────────────────────────────────
    if args.remove:
        _remove_document(args.remove, store, log)
        return
    ...
    for pdf_path in pdf_files:
        try:
            _process_pdf(pdf_path, store, log, force=(args.force == pdf_path.name))
            stats["processed"] += 1
...
def _process_pdf(
    ...
    # If forcing re-ingest, remove old chunks first
    if force and doc_id in log:
        logger.info(f"  Force re-ingesting {filename} — removing old chunks")
    ...
    logger.info(
        f"  Done: {len(chunks)} chunks from {len(pages)} pages in {elapsed}s"
    )
...
def _remove_document(filename: str, store: VectorStore, log: dict) -> None:
